...
- Decent Manual edits now leave rows exactly where they are placed.
- Manual edits show overlap warnings and disable Continue until the selected rows are valid for Disting.
- Non-XML Decent mapping modes export the full visible Low/Root/Vel/RR row map.
...
- Every real tag/group that points at samples stays visible; the user previews and selects the material.
...
- Merged developer `upstream/main` into `nymph-next-fix` at `28419624`, bringing in the v2.42.0+278 baseline plus Windows/video popup backend, focus, activation, and debug-diagnostics fixes.
